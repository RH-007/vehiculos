...
with tab2:
    ...
    if not df_a.empty and modelo1 != "(Elige)":
        ...
        oferta1 = colA.selectbox("Oferta (A) (orden: precio asc.)", opts_a if opts_a else ["(Sin ofertas)"], key="semi_oferta1")
    ...
    tipo2 = colB.selectbox("Tipo de Vehículo", ["(Elige)"] + sorted(df["tipo_vehiculo"].dropna().unique().tolist()), key="semi_tipo2")
    with colB:
    ...
    if not df_b.empty and modelo2 != "(Elige)":
        ...
        oferta2 = colB.selectbox("Oferta (B) (orden: precio asc.)", opts_b if opts_b else ["(Sin ofertas)"], key="semi_oferta2")
    ...
    # ======= TÍTULO COMPARATIVO =======
    if sel_a and sel_b:
        st.markdown(f"### Comparación: {sel_a.get('marca','')} {sel_a.get('modelo','')} vs {sel_b.get('marca','')} {sel_b.get('modelo','')}")
